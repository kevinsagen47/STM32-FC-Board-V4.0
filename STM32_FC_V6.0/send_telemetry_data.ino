...
void send_telemetry_data(void) {
  ...
  if (telemetry_loop_counter == 11) {
    ...
    //telemetry_buffer_byte=1000 + ((ground_pressure - pid_altitude_setpoint) * 0.0842);
    }
    else {
  ...
  if (telemetry_loop_counter == 17)telemetry_send_byte = start;//heading_lock;                      //Send the heading_lock variable as a byte.
