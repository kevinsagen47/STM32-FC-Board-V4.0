...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
float pid_p_gain_roll = 0.13;               //Gain setting for the pitch and roll P-controller (default = 1.3).
float pid_i_gain_roll = 0.0012;              //Gain setting for the pitch and roll I-controller (default = 0.04).
...
float pid_i_gain_pitch = 0.0012;  //Gain setting for the pitch I-controller.
float pid_d_gain_pitch = 7.0;  //Gain setting for the pitch D-controller.
...
uint8_t compass_address = 0x01E;            //The I2C address of the HMC5883L is 0x1E in hexadecimal form.
...
int8_t count1,count2;
int32_t mem1,mem2,mem3,mem4;
int32_t abs1,abs2,abs3,abs4;
int32_t now1,now2,now3,now4;
long time1,time2,curtime1,curtime2;
...
void setup() {
  ...
  pinMode(PB0, OUTPUT);                                         //Set PB3 as output for green LED.
  pinMode(PB1, OUTPUT);                                         //Set PB4 as output for red LED.
...
*/
  ...
  for (count_var = 0; count_var < 1250; count_var++) {          //1250 loops of 4 microseconds = 5 seconds.
    if (count_var % 125 == 0) {                                 //Every 125 loops (500ms).
      digitalWrite(PB1, !digitalRead(PB1));                     //Change the led status.
    }
    delay(4);                                                   //Simulate a 250Hz refresch rate as like the main loop.
  }
...
void loop() {
  ...
  if (channel_5 >= 1400 && channel_5 < 1800){
