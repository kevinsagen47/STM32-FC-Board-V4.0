...
void start_stop_takeoff(void) {
  if (channel_3 < 1050 && channel_6 > 1500 && start==0)start = 1;                              //For starting the motors: throttle low and yaw left (step 1).
  
    if(channel_3<1560 && channel_3>1440 && channel_5 >= 1400 && channel_5 < 2100 && channel_6 > 1500 && channel_2>1700 && start==0){
    start = 1;
    //initial_altitude_set=1;
    
    if(initial_altitude_set==0) {
      pid_altitude_setpoint=actual_pressure-15;
      initial_altitude_set=1;
    }
  ...
  if (start == 1 && channel_6 > 1500 && channel_2<1510 && channel_2>1490 && (flight_mode>=2 || channel_3<1050)) {     //When yaw stick is back in the center position start the motors (step 2).
    ...
    if (manual_takeoff_throttle > 1400 && manual_takeoff_throttle < 1600 && initial_altitude_set==0) {        //If the manual hover throttle is used and valid (between 1400us and 1600us pulse).
     ...
     else if (manual_takeoff_throttle > 1400 && manual_takeoff_throttle < 1600 && initial_altitude_set==1) {        //If the manual hover throttle is used and valid (between 1400us and 1600us pulse).
      takeoff_throttle = 20 ;                           //Use the manual hover throttle.
      takeoff_detected = 1;                                                        //Set the auto take-off detection to 1, indicated that the quadcopter is flying.
      //Reset the PID controllers for a smooth take-off.
      pid_i_mem_roll = 0;
      pid_last_roll_d_error = 0;
...
currenttimee=millis();
if(start==2 && channel_6<=1500){
  
}
else lastreceivee=currenttimee;
  
if(currenttimee-lastreceivee>=100){
  shutoff=1;
}


  if(start==2 && shutoff==1){
    start = 0;                                                                     //Set the start variable to 0 to disable the motors.
    takeoff_detected = 0; 
    shutoff=0;
    initial_altitude_set=0;
  }
