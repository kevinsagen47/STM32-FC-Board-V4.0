...
void red_led(int8_t level) {
  if (flip32)digitalWrite(PB1, !level);    //If a Flip32 is used invert the output.
  else digitalWrite(PB1, level);           //When using the BluePill the output should not be inverted.
}
void green_led(int8_t level) {
  if (flip32)digitalWrite(PB0, !level);    //If a Flip32 is used invert the output.
  else digitalWrite(PB0, level);           //When using the BluePill the output should not be inverted.
}
